...
import { signIn } from "../state/auth.ts"
    ...
    <div class="mx-auto grid max-w-4xl gap-6 lg:grid-cols-2">
      <div class="rounded-2xl border border-slate-800 bg-slate-900 p-6 sm:p-8">
        ...
        <div class="mt-6 space-y-4 text-sm text-slate-400">
          <div class="rounded-xl border border-slate-800 p-4">Secure sessions + MFA.</div>
          <div class="rounded-xl border border-slate-800 p-4">Realtime updates via WS.</div>
        </div>
      </div>
      <form
      ...
      >
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link href="/sign-up" class="text-sm text-indigo-400 hover:text-indigo-300">
            Create account
          </Link>
        </div>
